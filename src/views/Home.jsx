...
const Home = () => {
  ...
  // 🔄 Recarga forzada cada 20 minutos para mantener la app activa
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const requestWakeLock = async () => {
      try {
        if ("wakeLock" in navigator) {
          ...
          console.log("🔒 Pantalla bloqueada encendida (Wake Lock activo).");
        }
      } catch (err) {
    ...
    // Si la app vuelve a primer plano, vuelve a pedir el wake lock
  ...
  const handleVote = async (type) => {
    try {
      ...
      Swal.fire({
        ...
        icon: "success",
        confirmButtonColor: "#28a745",
        timer: 2000, // 👈 se cierra automáticamente en 5 segundos
        timerProgressBar: true, // 👈 muestra una barra de progreso visual
      });
  ...
  return (
    <div className="container">
      ...
      <div className="pt-2 text-center">
        <h2
          style={{ fontSize: "24px", marginBottom: "32px", fontWeight: "bold" }}
        >
          ¿CÓMO FUE TU EXPERIENCIA EN CEO?
        </h2>
        <h5>Selecciona una opción para votar.</h5>
      </div>
